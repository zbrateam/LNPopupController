...
- (void)updateConstraints
{
	if(_customBarViewController)
	{
		...
		CGFloat barHeight = _LNPopupBarHeightForBarStyle(_resolvedStyle, _isInlineWithTabBar, _customBarViewController);
...
- (void)layoutSubviews
{
    ...
    [_contentView insertSubview:_separatorView aboveSubview:_shadowView];
    ...
    _shadowView.frame = CGRectMake(0, self.isInlineWithTabBar ? 0.5 : 0, _toolbar.bounds.size.width, h);
    if (self.isInlineWithTabBar){
        _separatorView.frame = CGRectMake(0.5, 0, 1 / self.window.screen.nativeScale, _toolbar.bounds.size.height);
    } else {
...
- (void)_layoutTitles
{
	...
	if(_needsLabelsLayout == YES)
	{
		if(_titleLabel == nil)
		{
			...
			_titleLabel.font = _resolvedStyle == LNPopupBarStyleProminent ? [UIFont systemFontOfSize:13 weight:UIFontWeightRegular] : [UIFont systemFontOfSize:14];
		...
		if(_subtitleLabel == nil)
		{
			...
			_subtitleLabel.font = _resolvedStyle == LNPopupBarStyleProminent ? [UIFont systemFontOfSize:17 weight:UIFontWeightSemibold] : [UIFont systemFontOfSize:11];
...
{
	CGSize nextSize = CGSizeMake(size.width, _LNPopupBarHeightForBarStyle(_resolvedStyle, _isInlineWithTabBar, _customBarViewController));
